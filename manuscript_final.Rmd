...
#load the 16s data
load(here("data","b16s.cy.rData"))
b16s.cy<-b16s.cy%>%
  rename(Count = reads)%>%
  mutate(StationNum = as.character(StationNum),
         Presence = ifelse(Count == 0,"Nondetect","Detected"),
         Proportion = Count/TotalReads*100,
         Count = ifelse(Count==0,1,Count),
         Dayofyear = yday(Date))%>%
  left_join(volumes,join_by(StationNum==station,Date==date))%>%
  mutate(Date = as.Date(Date),
         countsperml = Count/volume)
...
rm(d2023,pn2023,names,rep_samps,site_codes,site_counts,totals,volumes)
...
```{r eukhabs, fig.width=8.5,fig.height=11, dpi=600,echo=FALSE,warning=FALSE}
...
#Pseudo-nitzschia plot
pseudo<-one8s%>%
  filter(Station %in% stn.list & Taxon %in% c("Pseudo-nitzschia"))%>%
  ...
  scale_x_date(limits = c(as_date(min(muss.l2$Date,na.rm = TRUE)),as_date(max(muss.l2$Date,na.rm = TRUE))),date_breaks = "1 year",date_labels = "%Y")+
  xlab(element_blank())+
  ylab(expression(atop(italic(Pseudo-nitzschia), "18S % of total reads")))+
  annotation_logticks()+
...
#domoic acid plot
DAplot<-muss.l2%>%
  filter(Region == "South Bay" & toxin == "DA" & SiteCode2!="LE") %>%
  mutate(value2 = ifelse(value2 == 0, 0.2,value2))%>%
  ggplot()+
...
chlplot<-petelong%>%
  ...
  guides(shape="none")+
  labs(col=element_blank())
fig3<-alex/pseudo/saxplot/DAplot/chlplot + 
  plot_layout(guides = "collect")+
  plot_annotation(tag_levels = "A")
fig3
```

## Fig. 5: Prokaryotic time series

Figure 5 represents combined time series of various cyanobacterial clades, microcystin and chl-a concentration

```{r prokhabs, fig.width=8.5,fig.height=7, dpi=600,echo=FALSE,warning=FALSE}
MCperps<-subset(habsuspects$Taxa,habsuspects$Group=="cyanobacteria") #list of cyanobacteria capable of producing microcystin

#filter out those suspects
MCmakers<-b16s.cy %>%
  filter(Taxa %in% MCperps,
         Region %in% c("Central Bay","South Bay"))

countmeas<-"Proportion" #or "Count" or "countsperml" or "Proportion"
axisname<-"16S % of total reads" #or "16S Count" "16S reads/mL" or "16S % of total reads"

#cyanobacteria plot
cyplot<-MCmakers %>%
  mutate(Proportion = case_when(Proportion<0.01~0.01,
                                TRUE~Proportion))%>%
  ggplot()+
  geom_point(aes_string("Date",countmeas,col="Taxa",shape="Presence"),size=2)+
  scale_y_continuous(trans="log10")+
  scale_x_date(date_breaks = "1 year",date_labels = "%Y")+
  xlab(element_blank())+
  ylab(axisname)+
  ...
  guides(shape="none")+
  labs(col=element_blank())
cylimit<-c(min(MCmakers$Date),max(MCmakers$Date)) #to limit the dates of the plot
muss.l2$Date<-as.Date(muss.l2$Date)

#microcystin plot
mctoxplot<-muss.l2%>%
  filter(Region == "South Bay" & toxin == "MC_total") %>%
  mutate(value2 = case_when(value2<0.1~0.1,
                            TRUE~value2))%>%
  ggplot()+
  geom_point(aes(Date,value2,col=SiteCode2,shape=Presence))+
  geom_hline(yintercept = 10,col="red")+
  scale_y_continuous(trans = "log10")+
  scale_x_date(date_breaks = "1 year",date_labels = "%Y",limits = cylimit)+
  annotation_logticks()+
  xlab(element_blank())+
  ylab("Mussel MC (ng/g)")+
  theme_bw(base_size = 14)+
...
fig5<-cyplot/mctoxplot/chlplot2 + 
  plot_layout()
fig5
```

## Figure S2: Seasonality of toxin concentration

Figure S2 shows how toxin concentrations vary by season

```{r boxplot, fig.width=8.5,fig.height=7, dpi=600,echo=FALSE,warning=FALSE}
...
seasonbox<-muss.l2%>%
  filter(Region %in% c("Central Bay","South Bay") & toxin %in% c("MC_total")) %>%
  mutate(value2 = case_when(value2<0.1~0.1,
                            TRUE~value2),
         toxin = "MC")%>%
  bind_rows(allSTX)%>%
  ...
  mutate(Season = case_when(Dayofyear>=213 & Dayofyear <=305 ~ 'fall', 
         ...
         Season = factor(Season,levels=c('winter','spring','summer','fall'),ordered = TRUE))%>%
  ggplot()+
  geom_boxplot(aes(Season,value2))+
...
## Figure S3: Chl-a vs toxin concentration by season

```{r chlvstox, fig.width=8.5,fig.height=7, dpi=600,echo=FALSE,warning=FALSE}
...
chltox<-muss.l2%>%
  ...
  facet_grid(toxin~Season,scales = "free")+
  theme_bw(base_size = 15)+
...
```{r qpcrvstox, fig.width=8.5,fig.height=11, dpi=600,echo=FALSE,warning=FALSE}
...
qtoxpn<-qpcr%>%
  filter(taxon=="Pseudo-nitzschia")%>%
  mutate(date=as_date(date))%>%
  full_join(allDA,join_by(closest(date <= Date),station==NearestPeterson),suffix=c(".qpcr",".tox"))%>%
  ...
  mutate(timediff = as.numeric(Date-date),
         ...
         value2 = ifelse(Presence == "Nondetect",1,value2),
         Dayofyear = yday(date),
         Season = case_when(Dayofyear>=213 & Dayofyear <=305 ~ 'fall', 
                            Dayofyear>=305 | Dayofyear<= 31 ~ 'winter',
                            Dayofyear>=32 & Dayofyear<=121 ~ 'spring',
         ...
         Season = factor(Season,levels=c('winter','spring','summer','fall'),ordered = TRUE))%>%
  filter(timediff<12)
pngam75<-qgam(log10(value2) ~ s(log10(qpcr)),qu=0.75,data=qtoxpn)
qtoxpn$pred75<-predict(pngam75,qtoxpn)
pngam9<-qgam(log10(value2) ~ s(log10(qpcr)),qu=0.9,data=qtoxpn)
qtoxpn$pred9<-predict(pngam9,qtoxpn)

#Pseudo-nitzschia vs DA plot
qtpnplot<-ggplot(qtoxpn)+
  geom_point(aes(qpcr,value2,shape=Presence),size=2)+
  #scale_color_viridis()+
  geom_line(aes(qpcr,10^pred9),col="red")+
  geom_line(aes(qpcr,10^pred75),col="orange")+
  scale_x_continuous("Pseudo-nitzschia (counts/mL)",trans = "log10")+
  scale_y_continuous("Mussel [DA] (ng/g)",trans = "log10")+
...
pn100q<-qpcr_cprplot("Pseudo-nitzschia",10)

fig4<-(qtaplot + qtpnplot)/(a300q + pn100q)/(qchlalexplot+qchlpnplot)+ plot_annotation(tag_levels = 'A')
fig4

```


## Figure 6: 18s vs toxin
...
```{r 18svstox, fig.width=8.5,fig.height=5.5, dpi=600,echo=FALSE,warning=FALSE}
...
```{r smalltox, fig.width=5,fig.height=5, dpi=1000,echo=FALSE,warning=FALSE}
cprplot.sm<-function(taxon,toxlevel){
  ...
  cprplot<-ggplot(cpr)+
    geom_point(aes(Proportion,toxprob))+
    geom_ribbon(aes(Proportion,ymin=toxthresh05,ymax=toxthresh95),col="gray",alpha=0.5)+
    scale_x_continuous("Alexandrium abundance",
                       trans = "log10",
                       breaks = c(0.01,1,100),
                       labels = c("0.01%","1%","100%"))+
...
## Figure S4: Equivalent of prior plot for cyano toxin vs 16S

```{r 16svstox, fig.width=8,fig.height=5, dpi=600,echo=FALSE,warning=FALSE}
#cyano plot
mcsuspects<-c("Microcystis","Dolichospermum","Planktothrix","Oscillatoriales")
allMC<-muss.l2%>%
  filter(toxin=="MC_total")
cy.cpr<-b16s.cy%>%
    filter(Taxa %in% mcsuspects)%>%
    dplyr::select(-Station)%>%
    dplyr::rename(Station=StationNum)%>%
    group_by(Date,Station)%>%
    summarise(Proportion=sum(Proportion))%>%
    #select the closest days matching in the toxin dataset, and matching according to whether they were previously joined as the nearest Peterson station
    left_join(allMC,join_by(Station==NearestPeterson),suffix=c(".16s",".tox"))%>%
    #exact chlorophyll matches by date and seasonz
    left_join(petechl,join_by(Date.16s == date,Station==station))%>%
    #create a time difference column (how many days after the Peterson was the mussel collected)
    mutate(timediff = as.numeric(Date.tox-Date.16s),
           toxthresh = as.numeric(ifelse(value2>10,1,0)))%>%
    #remove any matches greater than 14 days separated
    filter(timediff<14)%>%
    mutate(value2 = case_when(value2<=10~10,
                              TRUE~value2))%>%
    dplyr::select(Date.16s,toxthresh,Proportion,chl_merge,value2)%>%
    arrange(Proportion,toxthresh)%>%
    mutate(index = seq(nrow(.)-1,0,-1),
           toxprob = (sum(toxthresh)-cumsum(toxthresh))/index
    )%>%
  ...
  #bootstrap 1000 times, with replacement
  set.seed(123)
  test.cy<-bootstraps(cy.cpr,times=iterations,apparent = TRUE)
  ...
  #generate a blank data frame
  bci<-data.frame(ID = 1:33083)
  #list-apply the cplist function to all the entries in the bootstrapped object, to make a combined dataframe including all the CPAs side by side
  what<-lapply(test.cy$splits, cplist)
  for (i in seq(1,iterations+1,1)){
    bci[[paste("run",i,sep="")]]<-what[[i]]$toxprob.bs
  }
  ...
  percentiles_95<-apply(bci,1, function(row) quantile(row,probs = 0.975,na.rm=TRUE))
  percentiles_5<-apply(bci,1, function(row) quantile(row,probs = 0.025,na.rm=TRUE))
  cy.cpr$toxthresh95<-percentiles_95
  cy.cpr$toxthresh05<-percentiles_5
  
  concernthresh<-cy.cpr%>%
    filter(Proportion>0,
           toxthresh95>0.5)%>%
    arrange(toxthresh95)%>%
    head(1)
  cy.cpr<-cy.cpr%>%
    filter(index>=600)
  cprplot<-ggplot(cy.cpr)+
    geom_point(aes(Proportion,toxprob))+
    geom_ribbon(aes(Proportion,ymin=toxthresh05,ymax=toxthresh95),col="gray",alpha=0.5)+
    scale_x_continuous(paste("MCY producers: % of total reads",sep = ""),trans = "log10")+
    scale_y_continuous(paste("Prob. of mussels exceeding ",10," ng/g ","MCY",sep = ""))+
    theme_bw(base_size = 12)+  
    coord_cartesian(ylim=c(0,.25))
  cprplot

```
## Determining percentile associated with percent of reads threshold

```{r percentile,echo=FALSE,warning=FALSE}
#list of HAB taxa
Alexsub<-one8s%>%
  filter(Taxon=="Alexandrium")%>%
...
## Change in chlorophyll histograms through time 

```{r chlchange,echo=FALSE,warning=FALSE}
timeperiods<-petechl%>%
...
## Figure 7: Version that combines all seasons and the chlorophyll histograms

```{r CPAcomboallseasons,fig.width=8.5,fig.height=11, dpi=600,echo=FALSE,warning=FALSE}
...
print(doc, target = "baseline_probs.docx")
...
print(doc2, target = "taxon_thresholds.docx")
...
fig7<-cpacomp /histcomp + plot_layout(heights = c(9,1))
fig7
...
```{r smallmultiplot,fig.width=10,fig.height=4, dpi=1000,echo=FALSE,warning=FALSE}
...
## Figure 8 (Combined CPA for all seasons, quantifying nonlinear increases in probability)
...
```{r CPAgrouped4season,fig.width=8.5,fig.height=8, dpi=600,echo=FALSE,warning=FALSE}
library(EnvCpt)
library(gratia)
library(mgcv)
blanklistgroup<-list()
...
cprhabtop<-ggplot(habgroup)+
  ...
  theme(legend.position = "bottom",
        strip.background = element_rect(colour = "white", fill = "white"))
fig8<-cprhab/cprhabtop + plot_annotation(tag_levels = 'A')
fig8
```
